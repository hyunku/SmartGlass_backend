...
from listapp.serializers import BuildingTestSerializer
...
class BuildingTest(APIView):
    def get(self, request):
        queryset = Building.objects.all()
        serializer = BuildingTestSerializer(queryset, many=True)  # many=True : 여러개 객체 serialize 하기 위함
        return Response(serializer.data)
...
class Connect_user_glass(APIView):
    ...
    def put(self, request, pk):
        glass = self.get_object(pk)
        serializer = GlassSerializer(glass, data=request.data)
